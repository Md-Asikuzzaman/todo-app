...
const Edit: NextPage<Props> = ({ params }) => {
  ...
  try {
    if (id) {
      const data = useAppSelector((state) =>
        state.todoList.todo.find((t) => t.id === id)
      );
  ...
  return (
    <div
      onClick={handleClick}
      className={`fixed top-0 left-0 h-full bg-black/40 w-full items-center justify-center backdrop-blur-sm flex p-5`}
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className='bg-white p-5 max-w-[500px] w-full rounded-md shadow-md z-50'
      >
